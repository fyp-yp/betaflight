    ...
    if (!crcError) {
        if (!dataHandler.unsupported) switch (code) {
            ...
            case MSPCodes.MSP_RAW_IMU:
                ...
                if (Math.abs(FC.SENSOR_DATA.gyroscope[0]) > 1000 || Math.abs(FC.SENSOR_DATA.gyroscope[1]) > 1000 || Math.abs(FC.SENSOR_DATA.gyroscope[2]) > 1000) FC.CONFIG.gyroData = "fail";
                else if (!FC.CONFIG.gyroData) {
                    if (FC.SENSOR_DATA.gyroscope[0] == 0 && FC.SENSOR_DATA.gyroscope[1] == 0 && FC.SENSOR_DATA.gyroscope[2] == 0) FC.CONFIG.gyroData = "fail";
                    else FC.CONFIG.gyroData = "pass";
                }
